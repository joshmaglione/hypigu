...
\[
    {\sf HP}_{\mathcal{A}}^{\mathrm{sk}}(Y, T) = \dfrac{(1+ Y)^nE_n(T)}{(1 - T)^n},
\]
...
sage: S = hi.CombinatorialSkeleton(A)
sage: S.factor()
...
\[
    {\sf HP}_{\mathsf{D}_5}^{\mathrm{sk}} (1, T) = 1920\cdot \dfrac{1 + 26T + 66T^2 + 26T^3 + T^4}{(1 - T)^5}, 
\]
