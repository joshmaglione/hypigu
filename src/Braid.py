...
def _Igusa_braid_table(p, t, n, red=False):
    if n <= 0:
        return 1
    if n == 1:
        if red:
...
# of work. 
def _recursive_crank(p, t, n, known=[], red=False):
    if known == []:
        known = [_Igusa_braid_table(p, t, k, red=red) 
    ...
    for L in _partitions(k):
        if len(L) > 1:
            if red:
                L_factors = [_P(L), 1, t**(_beta(L)), _factorial(len(L))]
            else:
    ...
    known += [Zk]
    if k - 1 < n:
        return _recursive_crank(p, t, n, known=known, red=red)
    else:
        return known[n]

def BraidArrangement(n, reduced=False):
    from Globals import __DEFAULT_p, __DEFAULT_t
    p = _var(__DEFAULT_p)
    t = _var(__DEFAULT_t)
    if n <= _TABLE_CUTOFF:
        return _Igusa_braid_table(p, t, n, red=reduced)
    return _recursive_crank(p, t, n, red=reduced)
